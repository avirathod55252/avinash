        ...
        <div className="space-y-10">
          {experiences.map((exp, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-8 border border-blue-100 hover:-translate-y-1"
            >
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                <h3 className="text-2xl font-semibold text-gray-800">
                  {exp.role}
                </h3>
                <p className="text-sm text-blue-600 font-medium mt-2 sm:mt-0">
                  {exp.duration}
                </p>
              </div>
              ...
              {exp.companyUrl ? (
                <a
                  ...
                  className="text-lg font-semibold text-purple-600 hover:text-blue-700 underline underline-offset-4 transition-all duration-200 mb-3 inline-block"
                >
                  {exp.company}
                </a>
              ) : (
